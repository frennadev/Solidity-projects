...
contract Xing is Context, IERC20, Ownable {
    ...
    function openTrading() external onlyOwner() {
        ...
        // Compute and set the Uniswap V2 pair address for mainnet
        address factory = 0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f; // Uniswap V2 factory mainnet
        address weth = uniswapV2Router.WETH();
        address token0 = address(this) < weth ? address(this) : weth;
        address token1 = address(this) < weth ? weth : address(this);
        bytes32 initCodeHash = hex"96e8ac4277198ff8b6f785478aa9a39f403cb768dd02cbee326c3e7da348845f";
    ...
    function setTxLimitExempt(address account, bool exempt) external onlyOwner {
        require(account != owner(), "Owner must always be exempt");
        _isTxLimitExempt[account] = exempt;
    }

    // Allows the owner to compute and set the Uniswap V2 pair address for mainnet
    function setUniswapPairAuto() external onlyOwner {
        address factory = 0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f; // Uniswap V2 factory mainnet
        address weth = uniswapV2Router.WETH();
        address token0 = address(this) < weth ? address(this) : weth;
        address token1 = address(this) < weth ? weth : address(this);
