...
contract GBUSD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GBUSDDividendTracker public dividendTracker;
    ...
    constructor() ERC20("TEST", "$FRENNA") {
        marketingWallet = payable(0xaEc4d52511FCC14f970a1DE9053B497bb4Fb0714);
        devWallet = payable(0xaEc4d52511FCC14f970a1DE9053B497bb4Fb0714);
        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        payoutToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
        ...
        dividendTracker = new GBUSDDividendTracker(
            payable(this),
            router,
...
contract GBUSDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        GBUSDContract = GBUSD(mainContract);
    ...
    function _transfer(
        ...
        require(false, "GBUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "GBUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GBUSD contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(GBUSDContract);

            uint256 prevBalance = GBUSDContract.balanceOf(address(this));
            ...
            {
                uint256 received = GBUSDContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GBUSDContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
